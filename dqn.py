...
class DeepQNetwork():
    def __init__(self, sess, num_actions, state_shape, learning_rate=0.001, dropout_prob=0.2):
        ...
        depth = state_shape[2]
        self.x = tf.placeholder(tf.float32, [None, width, height, depth])
        ...
        self.Q_ = tf.placeholder(tf.float32, [None, num_actions])
    ...
    def train(self, Q, Q_):
        return self.sess.run(self.train_step, feed_dict={self.Q: Q, self.Q_: Q_})